...
**Document type:** Master technical plan (extended copy)  
**Audience:** Developer (including future self)  
**Note:** New file based on `READMEMAIN.md` — originals unchanged. Adds missing Overview entries + Phase 8.5 Frontend CSS.
...
- Phase 8.5 – Frontend CSS (Implementation)
...
Document the **actual CSS system** used in the app so new UI stays consistent with the notepad / scorekeeper design (Phase 8).
...
This chapter is the practical companion to Phase 8 (design intent).  
If design language and CSS disagree, update CSS deliberately — do not invent a second visual system.

---

## 2. Design tokens (`:root`)

| Token | Role | Current value / notes |
|-------|------|------------------------|
| `--paper` | Page background | `#fff8dc` |
| `--paper-strong` | Header / footer surface | `#f9e79f` |
| `--ink` | Primary text / structure | `#333` |
| `--blue` | Interaction (hover, focus, active) | `blue` |
| `--red` | Errors / danger / strong markers | `#c1121f` |
| `--font` | Handwritten UI font | `"Architects Daughter", cursive` |

Sketch assets (also on `:root`):

| Token group | Purpose |
|-------------|---------|
| `--sketch-a` / `--sketch-b` / `--sketch-c` | Hand-drawn box borders (black) |
| `--sketch-a-blue` / `--sketch-b-blue` | Same borders in interactive blue |
| `--underline` / `--underline-blue` | Input / link underlines |
| `--rule` / `--rule-gray` / `--rule-blue` | Table / separator rules |
| `--header-line` / `--footer-line` | Shell dividers |

**Rule:** prefer these tokens. Do not hardcode random hex colors in new feature CSS unless adding a new shared token.

---

## 3. Border system (mandatory pattern)

Borders are **not** `border: 1px solid …` cards.

Use only:

```html
class="border-frame border-frame--md"
```
...
|----------|-------------|
| `--sm` | buttons, nav chips, badges, compact controls |
| `--md` | panels, forms (default) |
| `--lg` | larger content blocks |
| `--xl` | hero / large frames |

### Optional stroke variants

- `border-frame--stroke-a`
- `border-frame--stroke-b`
- `border-frame--stroke-c`

### Important mechanics

- `border-image` + SVG sketch paths create imperfect edges
- `--frame-border` is in `em` so frames scale with font-size
- `--slice` is % of the **SVG source** (100×100), not of the element
- semantic layout classes (`stack-form`, `page`, …) do **not** draw borders — add `border-frame` separately

### Do / Don’t

**Do**
- compose: `button-link border-frame border-frame--sm`
- compose: `stack-form border-frame border-frame--md`
- alternate stroke variants lightly for organic feel

**Don’t**
- invent `sketchy-box` / `card` / `rounded-xl shadow` styles next to this system
- put `border-image` sketch borders on tiny checkboxes (they look broken) — checkboxes use solid ink border
- rely on perfect geometric `border-radius` for identity
...
| `.app-header` / `.app-footer` | Paper-strong bar + sketch divider lines |
| `.brand` | Product name; hover → blue |
| `.app-nav-row` | Nav row; links use `--sm` frames |
| `.app-main` | Main content padding |
| `.page` / `.page.narrow` | Page width |
| `.page-header` / `.page-header-actions` / `.page-primary-actions` | Title + action clusters |

Nav chips alternate stroke A/B; hover/active switch to blue sketch strokes.

---

## 5. Typography & base

- Global font: `var(--font)`
- Body: paper background + ink text
- `h1` has slight rotation (`transform: rotate(-0.4deg)`) — intentional notepad feel
- `.muted` → secondary text (`#555`)

Keep long tables readable; expressiveness belongs more in headings than dense data cells.
...
| Class | Role |
|-------|------|
| `.button-link` | Primary action look (pair with `border-frame--sm`) |
| `.button-secondary` | Secondary action stroke |
| `.linkish` | Text control with handwritten underline |
| main content `a` (not button/brand/frame) | Underlined ink → blue underline on hover |

Hover language: **blue stroke + blue text**, not fill/shadow.

### Forms

| Class / selector | Role |
|------------------|------|
| `.stack-form` | Vertical form stack |
| `.stack-form label` | Label stack |
| inputs / textarea / select | Transparent, underline background image |
| focus | `--underline-blue` |
| `.checkbox-row` | Checkbox + label row |
| `.sketch-check` / checkbox inputs | Solid ink border (no sketch border-image) |
| `.error-banner` / `.state-error` | Error surfaces (red + stronger stroke) |

### Tables & lists

| Class | Role |
|-------|------|
| `.data-table` | Sketch rule separators (header + row rules) |
| `.plain-list` | Simple list |
| `.row-actions` | Inline action group |
| `.scoreline` / `.timeline` / `.live-score` | Live match presentation |
| `.ws-connected` / `.ws-connecting` / `.ws-disconnected` / `.ws-error` | WebSocket status colors |

---

## 7. Feature / domain sections in CSS

These are feature layouts that still must speak the same visual language:

| Area | Classes (examples) |
|------|--------------------|
| Home | `.home-page`, `.home-lead` |
| Admin | `.admin-search`, `.admin-bulk`, `.admin-table-wrap`, `.admin-table` |
| Roles | `.role-fieldset`, `.role-check-grid` |
| Teams / shirts | `.shirt-color-*`, `.country-flag`, `.team-transfer*` |
| Field help | `.field-help*` |
| Phases / groups / knockout | `.phase-zone*`, `.group-stage-*`, `.knockout-*`, `.advancement-*`, `.phase-board*` |
| Group naming | `.group-list*`, `.group-list__name` (underline input) |

When adding a new screen:

1. reuse shell + `page` + `border-frame` sizes
2. reuse button/link/form primitives
3. only then add a small feature block section in `app.css`

---

## 8. Responsive behavior

Breakpoints already in CSS:

- `max-width: 900px` — team transfer collapses to one column
- `max-width: 640px` — tighter main padding, stacked page header, smaller frame borders/padding, slightly smaller nav text

**Rule:** keep sketch language on mobile; simplify spacing, do not switch to a flat “mobile Material” look.
...
- [ ] Uses `border-frame` + size modifier for framed surfaces
- [ ] Buttons/links use existing primitives (`button-link`, `linkish`, …)
- [ ] Forms use underline inputs / `stack-form` patterns
- [ ] Colors come from `:root` tokens
- [ ] Hover/focus uses blue stroke language
- [ ] No new card/shadow/radius system
- [ ] Feature-specific CSS lives in `app.css` under a clear section comment
- [ ] Mobile still readable; frames do not explode layout

---

## 10. Relationship to Phase 8

| Phase 8 (design) | Phase 8.5 (CSS) |
|------------------|-----------------|
| Atmosphere & do/don’t | Concrete classes & tokens |
| Suggests sketchy components | Actual names: `border-frame`, `button-link`, … |
| Palette & typography intent | `:root` variables in `app.css` |

Phase 8 remains the **why**.  
This chapter is the **how in this repo**.

---

## 11. Definition of Done (Phase 8.5)

This chapter / CSS system is “done” for ongoing work when:

- [ ] developers know to open `app.css` before inventing styles
- [ ] new screens compose existing frame + button + form primitives
- [ ] tokens stay the single source of colors/fonts/sketch assets
- [ ] Phase 8 mood still reads clearly across admin, preparation, and live views

---

**End of Phase 8.5 document**
